...
df = pd.read_csv('../../../data/TroFi/TroFi_formatted_all3737.csv')
verbs = df["verb"].iloc[0:3730]
frasi = df["sentence"].iloc[0:3730]
...
for i in range(10):
    ...
    model = torch.load('trofi_model_{}.pth'.format(i))
...
my_submission_preds.to_csv("trofi_preds.csv", index=False)
