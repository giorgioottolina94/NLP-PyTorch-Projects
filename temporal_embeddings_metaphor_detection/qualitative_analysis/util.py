...
def get_embedding_matrix2(word2idx, idx2word, normalization=False):
    """
...
def get_embedding_matrix(word2idx, idx2word, normalization=False):
    ...
    :return: an embedding matrix: a nn.Embeddings
    """
    # Load the Hist vectors into a dictionary, keeping only words in vocab
    embedding_dim = 100
    hist_path = "../hist/histwords.txt"
    hist_vectors = {}
    with open(hist_path) as hist_file:
        for line in tqdm(hist_file, total=get_num_lines(hist_path)):
            split_line = line.rstrip().split()
            word = split_line[0]
            if len(split_line) != (embedding_dim + 1) or word not in word2idx:
                continue
            ...
            if normalization:
                vector = vector / np.linalg.norm(vector)
            assert len(vector) == embedding_dim
            hist_vectors[word] = vector

    print("Number of pre-trained word vectors loaded: ", len(hist_vectors))

    # Calculate mean and stdev of embeddings
    all_embeddings = np.array(list(hist_vectors.values()))
    embeddings_mean = float(np.mean(all_embeddings))
    embeddings_stdev = float(np.std(all_embeddings))
    print("Embeddings mean: ", embeddings_mean)
    print("Embeddings stdev: ", embeddings_stdev)
    ...
    for i in range(2, vocab_size):
        word = idx2word[i]
        if word in hist_vectors:
            embedding_matrix[i] = torch.FloatTensor(hist_vectors[word])
    if normalization:
        for i in range(vocab_size):
            embedding_matrix[i] = embedding_matrix[i] / float(np.linalg.norm(embedding_matrix[i]))
    embeddings = nn.Embedding(vocab_size, embedding_dim, padding_idx=0)
...
def embed_indexed_sequence(sentence, pos_seq, word2idx, hist_embeddings, elmo_embeddings, bert_embeddings,
                           pos_embeddings=None):
    """
    ...
    if hist_embeddings is not None:
        ...
        hist_part = hist_embeddings(Variable(torch.LongTensor(indexed_sequence)))
    ...
    if bert_embeddings is not None:
        ...
        print(sentence)
        #assert (bert_part.shape == (len(words), 1024))
    ...
    # GLOVE+ELMO+POS:
    if elmo_embeddings is not None and pos_embeddings is not None and bert_embeddings is None and hist_embeddings is not None:
        result = np.concatenate((hist_part.data, elmo_part), axis=1)
        result = np.concatenate((result, pos_part.data), axis=1)
    # GLOVE:
    elif elmo_embeddings is None and pos_embeddings is None and bert_embeddings is None and hist_embeddings is not None:
        result = hist_part.data
    # GLOVE+POS:
    elif elmo_embeddings is None and pos_embeddings is not None and bert_embeddings is None and hist_embeddings is not None:
        result = np.concatenate((hist_part.data, pos_part.data), axis=1)
    # GLOVE+ELMO:
    elif elmo_embeddings is not None and pos_embeddings is None and bert_embeddings is None and hist_embeddings is not None:
        result = np.concatenate((hist_part.data, elmo_part), axis=1)
    # BERT:
    elif elmo_embeddings is None and pos_embeddings is None and bert_embeddings is not None and hist_embeddings is None:
        result = bert_part
    # GLOVE+BERT:
    elif elmo_embeddings is None and pos_embeddings is None and bert_embeddings is not None and hist_embeddings is not None:
        result = np.concatenate((hist_part.data, bert_part), axis=1)
    # GLOVE+ELMO+BERT:
    elif elmo_embeddings is not None and pos_embeddings is None and bert_embeddings is not None and hist_embeddings is not None:  
        result = np.concatenate((hist_part.data, elmo_part), axis=1)
        print(hist_part.data.shape)
        print(elmo_part.shape)
        print(result.shape)
        print(bert_part.shape)
...
class TextDatasetWithGloveElmoSuffix(Dataset):
    ...
    def collate_fn(batch):
        ...
        for pos, text, length, label in batch:
            ...
            padded_example_label = label + [-1] * amount_to_pad
