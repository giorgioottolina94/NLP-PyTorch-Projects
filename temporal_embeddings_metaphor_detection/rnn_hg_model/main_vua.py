...
embedded_train_vua = [[embed_indexed_sequence(example[0], example[2], word2idx,
                                      hist_embeddings, elmos_train_vua, bert_train_vua, suffix_embeddings),
                       example[2], example[1]]
                      for example in raw_train_vua]
embedded_val_vua = [[embed_indexed_sequence(example[0], example[2], word2idx,
                                    hist_embeddings, elmos_val_vua, bert_val_vua, suffix_embeddings),
                     example[2], example[1]]
...
'''
#RNN_HG model
RNNseq_model = RNNSequenceModel(num_classes=2, embedding_dim=1124, hidden_size=256, num_layers=1, bidir=True,
...
for epoch in range(additional_epochs):
    ...
    for (__, example_text, example_lengths, labels) in train_dataloader_vua:
        ...
        if num_iter % 3000 == 0:
            ...
            torch.save(RNNseq_model, 'vua_model_{}.pth'.format(i))
...
embedded_test_vua = [[embed_indexed_sequence(example[0], example[2], word2idx,
                                      hist_embeddings, elmos_test_vua, bert_test_vua, suffix_embeddings),
                       example[2], example[1]]
