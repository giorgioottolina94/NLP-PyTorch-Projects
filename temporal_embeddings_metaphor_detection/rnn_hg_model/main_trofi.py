...
bert_trofi = None
#bert_trofi = h5py.File('../bert/bert_trofi.hdf5', 'r')
#bert_trofi = np.load(open('../bert/bert_trofi.npy', 'r'))
...
embedded_trofi = [[embed_indexed_sequence(example[0], example[2], word2idx,
                                      hist_embeddings, elmos_trofi, bert_trofi, suffix_embeddings),
                       example[2], example[1]]
...
for i in range(10):
    ...
    RNNseq_model = RNNSequenceModel(num_classes=2, embedding_dim=1124, hidden_size=256,
    ...
    for epoch in range(num_epochs):
        ...
        for (__, example_text, example_lengths, labels) in train_dataloader_trofi:
            ...
            rnn_optimizer.step()
            num_iter += 1
            # Calculate validation and training set loss and accuracy every 150 gradient updates
            if num_iter % 150 == 0:
...
#                 print("Iteration {}. Training Loss {}.".format(num_iter, avg_eval_loss))
    ...
    torch.save(RNNseq_model, 'trofi_model_{}.pth'.format(i))
